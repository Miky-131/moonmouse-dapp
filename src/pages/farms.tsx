...
import { getLpPrice, getMMOUSEPriceFromLP } from '@/application/staking/getSignMessage'
  ...
  return (
    <>
      <div className="text-center justify-center mb-2">
        ...
        <br /> */}
        <p className="text-xl">Deposit LP tokens into Farms to earn additional rewards</p>
      </div>
...
function StakingCardCollapseItemFace({ open, info }: { open: boolean; info: IStakeInfo }) {
  ...
  useEffect(() => {
    async function fetchTokenPrice(tokenMint: string) {
      let resp;
      try {
        if (tokenMint == MMOUSETokenAddress) {
          resp = await getMMOUSEPriceFromLP(MMOUSELpAddress)
