...
export default function CoinInputBox({
  ...
  return (
    <Row
      className={twMerge(
        `relative flex-col border-solid border-1 border-[#d67aed7d] bg-field bg-[#1a0f1b] cursor-text rounded-xl py-3 px-6 mobile:px-4 ${disabled && !noDisableStyle ? 'pointer-events-none-entirely cursor-default opacity-50' : ''
        }`,
        className
      )}
      style={style}
    ...
    >
      ...
      <Row className="justify-between mb-2">
        ...
        <div
          className={`text-xs mobile:text-2xs justify-self-end text-color ${disabledInput ? '' : 'clickable no-clicable-transform-effect clickable-filter-effect'
            }`}
          onClick={() => {
