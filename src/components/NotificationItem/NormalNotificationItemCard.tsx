...
const existMs = process.env.NODE_ENV === 'development' ? 4 * 1000 : 4 * 1000 // (ms)
  ...
  return (
    ...
    >
      ...
      <Icon
        size="smi"
        heroIconName="x"
        className="absolute right-3 top-3 clickable text-gray-400 opacity-50 mobile:opacity-100 hover:opacity-100 transition-opacity"
        onClick={() => {
      ...
      <Row className="gap-3">
        ...
        <div>
          ...
          {description && (
            <div className="font-medium text-sm whitespace-pre-wrap mobile:text-xs text-gray-400">
              {description}
            </div>
          )}
        </div>
